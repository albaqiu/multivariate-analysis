...
As can be seen in the description of Dimensions, the \textbf{most significant variables in each dimension}, which are those represented with the highest value of R2, are the following: 
...
plot(si)
# 497 obs, we divide them to 6 clusters, we look for the max silhouette index: 3rd cluster
...
# Clusters based on factor cut
#table(dfClust$Cluster, dfClust$cut)
```
...
```{r}
# HCPC applied to the previously computed PCA
res.hcpc <- HCPC(diamonds.pca.sup, nb.clust = 4) # 4 clusters
...
#### MCA-based clustering

The Hierarchical Clustering on Principal Components (HCPC) method is also applied to the individual scores obtained from the MCA. As in the PCA case, this approach performs Ward’s hierarchical clustering on the low-dimensional representation provided by the MCA, so that the clusters reflect the joint structure of the categorical transformations of the numeric variables (over/under average) and the proportion-based factors (table and depth).

Using the \emph{HCPC()} function is used with an automatic cut leads to a six-cluster solution. However, the factor map shows that these six groups are largely overlapping and mainly separated along Dimension 1, which reflects the size gradient introduced by the over/under transformations of the numerical variables. Dimension 2, linked to geometric proportions (Ideal vs. NotIdeal table and depth, and cut), adds some vertical dispersion, but the clusters remain close to each other and difficult to distinguish.

```{r}
# HCPC applied to the previously computed MCA
hc_mca <- HCPC(res.mca, -1) # automatic cut: 6 clusters

# Dendrogram visualization
library(factoextra)
fviz_dend(hc_mca, rect = TRUE, rect_fill = TRUE)

# Cluster visualization on the PCA factor map
fviz_cluster(hc_mca, repel = TRUE, show.clust.cent = TRUE,
             main = "Clusters on MCA Factor Map")
```

Although the automatic HCPC cut suggests a 6-cluster partition, the factor map and the dendrogram reveal that a 4-cluster solution is more balanced and more interpretable. 

Based on the cluster distribution on the MCA factor map, the characteristics of each group can be summarised as follows: !!!!!!!!!!!

```{r}
# HCPC applied to the previously computed MCA
hc_mca <- HCPC(res.mca, nb.clust = 4) # 4 clusters
