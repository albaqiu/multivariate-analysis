...
!!!!!!!!!!!!!!!!!!!!!!! Comença aquí el que he afegit
Apart from the previous analysis, numerical variables were projected onto the Gower-based configuration in order to aid interpretation of the latent dimensions, while categorical variables were used to explain the observed grouping structure. Only variables that provide clear and consistent interpretative value are discussed.

The projection of logCarat and logPrice onto the MDS map reveals a strong and coherent gradient along the first coordinate. Diamonds located on the left side of the configuration exhibit higher values of both logCarat and logPrice, whereas those on the right correspond to smaller and less valuable diamonds. This confirms that Dimension 1 primarily reflects overall physical size and economic value, in agreement with the first PCA component, which captured the joint variation of carat, physical dimensions, and price.

The physical dimensions x, y, and z show very similar spatial patterns to logCarat, reinforcing this interpretation. Higher values of x, y, and z are consistently associated with negative values of the first coordinate, indicating that this axis captures a global size effect rather than isolated geometric characteristics. The strong alignment between these variables suggests that the size-related information is redundantly encoded, justifying their joint interpretation as a single physical magnitude dimension.

In contrast, Dimension 2 is mainly associated with variables describing geometric proportions. The projection of depth shows a positive gradient along the second coordinate, while table exhibits an opposite trend.(????not really pero ns que dir, podem no dir res i no incloure les variables) Diamonds with higher depth values tend to occupy the upper part of the configuration, whereas diamonds with larger table values are located lower on the map. This pattern indicates that Dimension 2 captures a geometric balance between taller, more pointed diamonds and flatter, wider ones, which is consistent with the interpretation of the second PCA component.
...
```
When projecting the categorical variables onto the Gower MDS configuration, the cut variable shows the strongest association with the separation of groups, particularly along the second coordinate. Diamonds with Ideal cut form a compact cluster in the upper region of the map, while Premium diamonds are mainly located in the lower region, indicating that cut quality is a key driver of the vertical separation between clusters. In contrast, Very Good and Fair/Good cuts are more dispersed and occupy intermediate positions, reflecting a gradual transition rather than sharp separation.

The color variable exhibits a weaker structure, different color grades overlap substantially, but higher-quality colors (D–F) tend to concentrate toward the upper part of the configuration, whereas lower-quality colors (G–I/J) appear more frequently in the lower region. Similarly, clarity shows a similar situation, higher clarity grades (VVS2, IF/VVS1) are more prevalent in the upper clusters, while lower clarity categories (I1/SI2, SI1) are more common in the lower part of the map. However, the considerable overlap between clarity levels.

We had observed before four compact groups in the Gower-based MDS configuration, these groups are not fully explained by any single categorical variable. The projections of cut, color, and clarity reveal partial alignment with the observed separation, particularly along the second dimension, but do not correspond with the visually identified clusters. This indicates that the clustering structure arises from the combined contribution of multiple numerical and categorical variables rather than being driven by a single categorical attribute.
