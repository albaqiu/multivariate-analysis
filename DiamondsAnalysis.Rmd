...
For each variable, we check: duplications, existence of zeros, existence of outliers, existence of missing values and we apply transformations if it is needed.

### Carat variable (numeric)
...
Same as before, we analyse the different levels of color, the most frequent is G color grade and J color grade is the less frequent, meaning diamonds with a noticeable tint are less common in the dataset.
It can be mapped to 1,2,3..??? (es una escala)
...
```
### Depth variable
The depth variable has no NA and no zeros, it has 2 extreme outliers, which we removed.
...
```
### Table variable
The variable table has no NA, zeros or outliers.
Also it is not normally distributed but and since lambda = -1 we apply reciprocal transformation (1/table).
...
abline(v=lsout,col='orange')
sev_out #0
diamonds <- diamonds[-sev_out,] # Remove extreme outliers (but there's not outliers)
...
curve(dnorm(x,m,std),add=T, col="red")
shapiro.test(diamonds$table) #p-value < 0.01 -> not normally distirbuted

# Transformation
boxcox(diamonds$table~1,lambda=seq(-1,1,by=0.1)) # Box-Cox Transformation
# Reciprocal transformation based on the Box-Cox suggestion (lambda = -1)
