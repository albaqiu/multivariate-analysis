...
First, clustering is applied using only the numerical variables, combining hierarchical and non-hierarchical methods, as well as clustering based on the principal component scores.
Second, clustering of numerical and categorical variables is conducted to obtain a more comprehensive segmentation of the data.
...
The ratio between the between-cluster sum of squares and the total sum of squares is computed. For $k=4$, this ratio is equal to $0.694$, indicating a fair degree of separation between clusters. For $k=6$, the ratio increases to $0.768$, suggesting a better-defined cluster structure.
...
# Optimum point: k=6 (minimization of TWSS)
...
k4$betweenss/k4$totss # as it gets closer to 1 better --> better separation of clusters
...
k6$betweenss/k6$totss # as it gets closer to 1 better --> better separation of clusters
...
plot(si)
# 497 obs, we divide them to 6 clusters, we look for the max silhouette index: 1st cluster
