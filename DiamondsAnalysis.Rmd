...
# ---------------------------
# Supplementary qualitative modalities (logPrice_*)
# ---------------------------
supq <- NULL
if (!is.null(res.mca$quali.sup) && !is.null(res.mca$quali.sup$coord)) {
  ...
  # Keep only logPrice modalities (e.g., logPrice_b, logPrice_o, etc.)
...
# x, y, z, logCarat (your labels look like x_b, x_o, etc.)
...
# cut / depth / table (these DO have prefixes in your plot)
...
# ---------------------------
# Label size (make key groups bigger)
# ---------------------------
cat$lab_size <- ifelse(cat$group %in% c("cut","depth","table","x","y","z","logCarat"), 4.2, 3.2)

# ---------------------------
# Color palette (soft green + blue, low clash)
# xyz/logCarat same green; others green/blue shades
# ---------------------------
pal <- c(
  x        = "#1b7837",
  y        = "#1b7837",
...
# ---------------------------
# Square limits (include supplementary if present)
# ---------------------------
all_dim1 <- cat$Dim1
all_dim2 <- cat$Dim2
...
p <- ggplot(cat, aes(Dim1, Dim2)) +
  ...
  scale_size_identity() +
  coord_fixed(xlim = c(-m, m), ylim = c(-m, m)) +
...
# ---------------------------
# Add supplementary logPrice modalities (move labels up/down to improve readability)
# ---------------------------
if (!is.null(supq) && nrow(supq) > 0) {
  # If you have exactly two modalities (e.g., _b and _o), separate them vertically
  supq$nudge_y <- 0
  ...
  # If the suffix isn't _b/_o, just push all logPrice labels slightly upward
