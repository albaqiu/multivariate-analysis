...
author: "Laia Jané, Elisa Müller, Runxiao Qiu and Berta Torrents"
date: "December 2025"
output:
  pdf_document:
...
The color variable shows no missing values and is relatively well balanced. Nevertheless, the most frequent grade is G, representing $12.6\%$ of the observations, while the least frequent grade is J, accounting for $3.4\%$, indicating that diamonds with a noticeable tint are less common in the dataset. To avoid categories with very low frequencies and to improve stability in the multivariate analysis, the light-tinted grades I and J have been grouped together. These grades are consecutive on the GIA color scale and represent similar coloration.
...
## 1.2. Correlations between numeric variables
...
barplot(dimaonds.mds.eu$eig[1:6], names.arg = 1:6, main = "Scree Plot (Euclidean distance)", xlab = "Dimension", ylab = "Eigenvalue", col = "blue")
lines(dimaonds.mds.eu$eig, type = "o", col = "red", pch = 19, add=T)
...
barplot(diamonds.mds.gw$eig[1:8], names.arg = 1:8, main = "Scree Plot (Gower distance)", xlab = "Dimension", ylab = "Eigenvalue", col = "blue")
lines(diamonds.mds.gw$eig, type = "o", col = "red", pch = 19, add=T)
...
### Understanding Euclidean and Gower coordinates

```{r}
# Euclidean coordinates
mds_eu_coords <- as.data.frame(dimaonds.mds.eu$points)
colnames(mds_eu_coords) <- c("Dim1", "Dim2")

mds_eu_coords$logCarat <- diamonds$logCarat
mds_eu_coords$logPrice <- diamonds$logPrice
mds_eu_coords$depth <- diamonds$depth
mds_eu_coords$table <- diamonds$table
mds_eu_coords$x <- diamonds$x
mds_eu_coords$y <- diamonds$y
mds_eu_coords$z <- diamonds$z
mds_eu_coords$cut <- diamonds$cut
mds_eu_coords$color <- diamonds$color
mds_eu_coords$clarity <- diamonds$clarity

# Gower coordinates
mds_gw_coords <- as.data.frame(diamonds.mds.gw$points)
colnames(mds_gw_coords) <- c("Dim1", "Dim2")

mds_gw_coords$logCarat <- diamonds$logCarat
mds_gw_coords$logPrice <- diamonds$logPrice
mds_gw_coords$depth <- diamonds$depth
mds_gw_coords$table <- diamonds$table
mds_gw_coords$x <- diamonds$x
mds_gw_coords$y <- diamonds$y
mds_gw_coords$z <- diamonds$z
mds_gw_coords$cut <- diamonds$cut
mds_gw_coords$color <- diamonds$color
mds_gw_coords$clarity <- diamonds$clarity
```

To assess whether the Euclidean coordinates are related to the PCA dimensions, the numerical variables have been projected onto the Euclidean-based configuration.

As expected, the variables x, y, z, logCarat and logPrice display a clear colour gradient along the first coordinate. This reinforces the interpretation that this coordinate represents overall physical magnitude. The gradient ranges from lighter to darker tones, indicating that smaller and less expensive diamonds are located on the left side of the axis, whereas larger and more expensive diamonds are positioned on the right.

In addition, a clear vertical gradient is observed for the depth and table variables, suggesting that the second coordinate is also related to the second PCA dimension and conveys information about diamond geometry. Specifically, diamonds with greater depth and more pointed shapes tend to appear in the upper part of the map, while shallower diamonds with larger table values are located toward the lower part.

```{r}
# logCarat
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = logCarat)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightyellow", high = "darkred") +
  labs(title = "Euclidean MDS: logCarat",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# logPrice
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = logPrice)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightblue", high = "navy") +
  labs(title = "Euclidean MDS: logPrice",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# depth
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = depth)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightgreen", high = "darkgreen") +
  labs(title = "Euclidean MDS: Depth",
       x = "Coordinate 1", y = "Coordinate 2", color = "Depth") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# table (purple monochrome, not similar to logCarat)
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = table)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lavender", high = "purple4") +
  labs(title = "Euclidean MDS: Table",
       x = "Coordinate 1", y = "Coordinate 2", color = "Table") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# x
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = x)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightcyan", high = "darkcyan") +
  labs(title = "Euclidean MDS: x (length)",
       x = "Coordinate 1", y = "Coordinate 2", color = "x") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# y
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = y)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "peachpuff", high = "saddlebrown") +
  labs(title = "Euclidean MDS: y (width)",
       x = "Coordinate 1", y = "Coordinate 2", color = "y") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# z
ggplot(mds_eu_coords, aes(Dim1, Dim2, color = z)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightgray", high = "black") +
  labs(title = "Euclidean MDS: z (height)",
       x = "Coordinate 1", y = "Coordinate 2", color = "z") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```

In order to identify the relationship between the Gower coordinates and the variables, the same approach as before has been followed: both numerical and categorical variables have been projected onto the Gower-based configuration.

The projections of logCarat, logPrice, x, y and z display a strong and coherent gradient along the first coordinate, suggesting that this dimension is closely related to the overall physical size and economic value of a diamond. In this configuration, the orientation is reversed with respect to previous representations, since diamonds located on the left side of the map exhibit higher values of these variables, whereas those on the right correspond to smaller and less valuable diamonds. This confirms that the first Gower dimension primarily reflects overall physical magnitude and price, in line with the first PCA component, which captured the joint variation of weight, size dimensions and market value.

In contrast, the second Gower dimension is mainly associated with variables describing geometric proportions. Although the projection of depth does not show a sharply defined gradient, higher depth values tend to appear in the upper part of the map, while lower depths are more frequent in the lower region. Conversely, table exhibits an opposite pattern, with larger table values concentrating toward the lower side of the configuration and smaller table values toward the upper side. Together, these patterns indicate that the second dimension reflects variation in diamond geometry, distinguishing between more pointed diamonds with greater depth and shallower diamonds characterized by larger table values, in line with the interpretation of the second PCA component.

On the other hand, when the categorical variables are projected onto the Gower-based MDS configuration, the cut variable shows the strongest association with the separation of observations, particularly along the second coordinate. Diamonds with an Ideal cut concentrate in the upper region of the map, suggesting an association with more pointed geometric profiles, whereas Premium diamonds are mainly located in the lower region, indicating a closer relationship with flatter geometries. In contrast, the Very Good and Fair/Good categories appear more dispersed and occupy intermediate positions, reflecting a gradual transition rather than a sharp separation between cut levels.

The color and clarity variables exhibit a much weaker structure. Their categories form diffuse clouds without clear separation, indicating that these attributes contribute less to explaining the variability of the Gower coordinates and play a secondary role in shaping the overall configuration.

Finally, when relating these results to the four compact groups identified in the Gower-based MDS configuration, it can be seen that the clusters are mainly organized according to diamond size, price, and geometric proportions. Each cluster occupies a distinct quadrant of the map, reflecting different structural profiles. Specifically, the green cluster corresponds to small and inexpensive, more pointed diamonds; the orange cluster to small and inexpensive, shallower diamonds; the blue cluster to large and expensive, flatter diamonds; and the red cluster to large and expensive, more pointed diamonds. This indicates that the clustering structure arises from the joint effect of several numerical and categorical variables, rather than being explained by any single categorical attribute alone.

```{r}
# logCarat
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = logCarat)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightyellow", high = "darkred") +
  labs(title = "Gower MDS: logCarat",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# logPrice
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = logPrice)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightblue", high = "navy") +
  labs(title = "Gower MDS: logPrice",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# depth
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = depth)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightgreen", high = "darkgreen") +
  labs(title = "Gower MDS: Depth",
       x = "Coordinate 1", y = "Coordinate 2", color = "Depth") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

# table (purple, same as Euclidean)
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = table)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lavender", high = "purple4") +
  labs(title = "Gower MDS: Table",
       x = "Coordinate 1", y = "Coordinate 2", color = "Table") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# x
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = x)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightcyan", high = "darkcyan") +
  labs(title = "Gower MDS: x (length)",
       x = "Coordinate 1", y = "Coordinate 2", color = "x") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# y
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = y)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "peachpuff", high = "saddlebrown") +
  labs(title = "Gower MDS: y (width)",
       x = "Coordinate 1", y = "Coordinate 2", color = "y") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# z
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = z)) +
  geom_point(size = 2, alpha = 0.8) +
  scale_color_gradient(low = "lightgray", high = "black") +
  labs(title = "Gower MDS: z (height)",
       x = "Coordinate 1", y = "Coordinate 2", color = "z") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
# cut
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = cut)) +
  geom_point(size = 2, alpha = 0.8) +
  labs(title = "Gower MDS: Cut",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# color
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = color)) +
  geom_point(size = 2, alpha = 0.8) +
  labs(title = "Gower MDS: Color",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))

# clarity
ggplot(mds_gw_coords, aes(Dim1, Dim2, color = clarity)) +
  geom_point(size = 2, alpha = 0.8) +
  labs(title = "Gower MDS: Clarity",
       x = "Coordinate 1", y = "Coordinate 2") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```

In this context, MDS has proven to be a valuable complementary method to PCA, providing an alternative, distance-based perspective on the structure of the diamonds dataset. The strong consistency between the Euclidean MDS and PCA representations reinforces the robustness of the main dimensions identified using numerical variables, which are primarily related to overall size and price. In contrast, the Gower-based MDS reveals additional structure arising from the inclusion of categorical variables, leading to a clearer separation of observations. In particular, geometric differences become more apparent: diamonds with an Ideal cut tend to be associated with more pointed shapes, while Premium cuts are more closely linked to shallower geometries. These patterns contribute to well-defined groupings that are not captured by numerical information alone. Overall, MDS enhances the interpretability of the data and supports the understanding and validation of the clustering results when working with correlated variables and mixed data types.
...
The MCA analysis has been applied using the \emph{MCA()} function from the \emph{FactoMineR} library on the original categorical variables and the categorized numerical variables. The objective of this method is to identify global patterns, associations and similarities among the categories of different variables. By including the categorized numerical variables, the analysis is more complete than the results obtained from CA. Several strategies were tested to categorize the numeric variables, and the best results are those presented in this report, selecting the following for maximum interpretability.

Most of the numerical variables have been transformed into binary factors using the median ("above average" / "below average") to reduce dimensionality.  However, the table and depth variables have been categorized based on gemological criteria, as their interpretation depends on proportion ranges that determine whether a diamond is well-cut.
...
In order to select the dimensions to be interpreted, one possible approach is to retain those with eigenvalues greater than the average inertia of the indicator matrix. In this case, the dataset contains $J = 9$ variables and a total of $K = 30$ categories, resulting in $K - J = 21$ possible dimensions. The corresponding average inertia is therefore $1/21 \approx 0.048$, which would theoretically suggest retaining dimensions up to Dimension $17$. However, this criterion is known to be overly permissive in MCA and does not necessarily ensure interpretability.

Although the scree plot shows a noticeable elbow around Dimension $2$, a more informative criterion in MCA consists in examining whether the relevant variables are adequately represented across the extracted dimensions. From this perspective, it would be reasonable to retain dimensions up to Dimension $4$, as all variables appear at least once as relevant contributors.

Nevertheless, despite Dimensions $3$ and $4$ being mainly associated with the categorical variables color and clarity, their graphical interpretation does not reveal clear or well-defined separation patterns among the individuals. The corresponding categories are highly overlapping and do not form distinct regions in the factor space. Consequently, these dimensions mainly capture secondary variation rather than meaningful global structure. For this reason, the analysis focuses on the first two dimensions, which provide the most interpretable and informative representation of the data.
...
res.mca$eig
barplot(res.mca$eig[1:19], names.arg = 1:19, main = "Scree Plot", xlab = "Dimension", ylab = "Eigenvalue", col = "blue")
lines(res.mca$eig[1:19], type = "o", col = "red", pch = 19, add=T)
...
Additional plot with ggplot2 for better interpretability

```{r mca_categories_grouped_pretty, fig.width=7, fig.height=7, message=FALSE,warning=FALSE}
library(ggplot2)
library(ggrepel)

cat <- as.data.frame(res.mca$var$coord[, 1:2])
colnames(cat) <- c("Dim1", "Dim2")
cat$label <- rownames(cat)

supq <- NULL
if (!is.null(res.mca$quali.sup) && !is.null(res.mca$quali.sup$coord)) {
  supq <- as.data.frame(res.mca$quali.sup$coord[, 1:2, drop = FALSE])
  colnames(supq) <- c("Dim1", "Dim2")
  supq$label <- rownames(supq)

  supq <- supq[grepl("^logPrice", supq$label), , drop = FALSE]
}

pct <- round(res.mca$eig[1:2, 2], 2)

cat$group <- "other"

cat$group[grepl("^x_", cat$label) | cat$label == "x"] <- "x"
cat$group[grepl("^y_", cat$label) | cat$label == "y"] <- "y"
cat$group[grepl("^z_", cat$label) | cat$label == "z"] <- "z"
cat$group[grepl("^logCarat", cat$label) | cat$label == "logCarat"] <- "logCarat"

cat$group[grepl("^cut_", cat$label)]   <- "cut"
cat$group[grepl("^table_", cat$label)] <- "table"
cat$group[grepl("^depth_", cat$label)] <- "depth"
cat$group[cat$label %in% levels(dfmca$color)]   <- "color"
cat$group[cat$label %in% levels(dfmca$clarity)] <- "clarity"

cat$lab_size <- ifelse(
  cat$group %in% c("cut","depth","table","x","y","z","logCarat"),
  4.8,  
  3.8   
)
...
all_dim1 <- cat$Dim1
all_dim2 <- cat$Dim2
if (!is.null(supq) && nrow(supq) > 0) {
  all_dim1 <- c(all_dim1, supq$Dim1)
  all_dim2 <- c(all_dim2, supq$Dim2)
}
m <- max(abs(c(all_dim1, all_dim2)))

p <- ggplot(cat, aes(Dim1, Dim2)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_point(aes(color = group), shape = 17, size = 2, alpha = 0.85) +
  geom_text_repel(
    aes(label = label, color = group, size = lab_size),
    box.padding = 0.35,
    point.padding = 0.25,
    force = 2,
  ...
  scale_size_identity() +
  coord_fixed(xlim = c(-m, m), ylim = c(-m, m), clip = "off") +
  scale_color_manual(values = pal) +
  labs(
    title = "Graph of the active categories",
    x = paste0("Dim 1 (", pct[1], "%)"),
    y = paste0("Dim 2 (", pct[2], "%)")
  ) +
...
if (!is.null(supq) && nrow(supq) > 0) {
  supq$nudge_y <- 0
  if (any(grepl("_b$", supq$label))) supq$nudge_y[grepl("_b$", supq$label)] <-  0.28
  if (any(grepl("_o$", supq$label))) supq$nudge_y[grepl("_o$", supq$label)] <- -0.28

  if (all(supq$nudge_y == 0)) supq$nudge_y <- 0.22

  p <- p +
    geom_point(
      data = supq, aes(Dim1, Dim2),
      inherit.aes = FALSE,
...
Nevertheless, some individuals remain close to the centre of the map (e.g., individuals 338, 297 and 82). These observations illustrate mixed profiles: they combine categories that pull in opposite directions (for instance, above-average in some size-related variables but below-average in others), and they also tend to mix ideal and non-ideal geometric characteristics (e.g., \textit{table_Ideal} together with \textit{depth_High}). As a consequence, no single modality dominates their profile and their coordinates remain close to zero.

```{r mca_ind_marked, fig.width=6, fig.height=6, message=FALSE, warning=FALSE}
coord <- res.mca$ind$coord[, 1:2]
pct <- round(res.mca$eig[1:2, 2], 2)

# Base factor map
plot(coord,
     pch = 16, cex = 0.6,
     xlab = paste0("Dim 1 (", pct[1], "%)"),
     ylab = paste0("Dim 2 (", pct[2], "%)"),
     main = "MCA factor map (Individuals)")
...
text(coord[idx, 1], coord[idx, 2],
     labels = idx, col = "blue", pos = 3, cex = 0.9)
...
```{r}
# Coordinates of individuals on the first two MCA dimensions
ind_coord <- res.mca$ind$coord[, 1:2]

# Distance to the origin
dist0 <- sqrt(ind_coord[,1]^2 + ind_coord[,2]^2)

# Individuals closest to the origin
head(sort(dist0), 6)

```

```{r}
# Inspect categorical profiles of central individuals
dfmca[names(head(sort(dist0), 6)), ]

# Inspect numerical profiles of central individuals
diamonds[names(head(sort(dist0), 6)), ]
```
...
Dimension $3$ shows additional variability mainly associated with cut and depth. However, this dimension largely reflects a refinement of the geometric effects already captured by Dimension $2$ rather than introducing a new independent structure. The associated categories are spread across the axis without forming clearly separated groups of individuals.

Dimension $4$ is primarily driven by the categorical variables color and clarity. Although these variables contribute more strongly to this dimension, the corresponding categories display substantial overlap in the factor space. As a result, Dimension $4$ does not lead to a clear separation of individuals and mainly captures secondary variation related to internal quality attributes.

Overall, while Dimensions $3$ and $4$ contribute to completing the representation of all variables, they do not reveal additional interpretable or well-defined patterns beyond those already identified in the first two dimensions.
...
### Clouds ###
## Cloud of Individuals
plot.MCA(res.mca,choix="ind", axes = c(3, 4), label="none")

## Cloud of Individuals
plot.MCA(res.mca,choix="ind",axes = c(3, 4), label="none",invisible="var",habillage = "clarity") #Dim 4
plot.MCA(res.mca,choix="ind",axes = c(3, 4), label="none",invisible="var",habillage = "color")   #Dim 4
plot.MCA(res.mca,choix="ind",axes = c(3, 4), label="none",invisible="var",habillage = "cut")     #Dim 3
plot.MCA(res.mca,choix="ind",axes = c(3, 4), label="none",invisible="var",habillage = "depth")   #Dim 3

## Cloud of Categories
plot(res.mca,invisible=c("ind"),axes = c(3, 4), title="Graph of the active categories")

## Cloud of variables
plot(res.mca,choix="var",axes = c(3, 4),title="Cloud of variables")
...
Despite their superior clarity and cut, diamonds in this cluster exhibit significantly smaller physical dimensions (x, y and z) and lower logCarat values. The logPrice variable is also well below the overall mean. These diamonds tend to present slightly higher depth values and smaller table values, indicating more pointed stones with narrower top surfaces, which is consistent with their smaller overall size. 
...
This cluster accounts for $27.6\%$ of the sample. It contains diamonds that are moderately larger and more expensive than average, as indicated by significantly positive deviations in logCarat, logPrice, and all three physical dimensions (x, y and z). Depth values are also slightly above the overall mean, while table values are marginally lower. It should be highlighted that the numerical deviations are moderate.
...
This cluster accounts for $15.3\%$ of the sample and is clearly distinguished by the largest and most valuable diamonds in the dataset. All size-related variables (x, y, z and logCarat), together with logPrice, show very large positive deviations from the overall mean. In addition, the table variable also presents a mean value above the overall average.
...
As the resulting p-value is significantly lower than the significance level $\alpha$, the null hypothesis of homogeneity of variances is rejected. Furthermore, an inspection of the boxplots reveals clear differences in the variance homogeneity across the depth and table variables.
...
When evaluated on the train set, the QDA achieves an overall correct classification rate (CCR) of $68.2\%$, which represents a substantial improvement over random classification and indicates good predictive performance given the limited number of predictors. In particular, the model performs very well for the most prevalent and well-defined categories, Ideal and Premium, with class-specific accuracies of $91.8\%$ and $83.5\%$, respectively. This suggests that depth and table are especially informative for identifying higher-quality cuts, where geometric differences are more pronounced. In contrast, classification performance is considerably lower for Fair/Good ($22.2\%$) and Very Good ($22.7\%$) cuts.
...
When evaluated on the test set, the QDA achieves an overall correct classification rate (CCR) of $64.6\%$, which represents a substantial improvement over random classification and indicates good predictive performance given the limited number of predictors. In particular, the model performs very well for the most prevalent and well-defined categories, Ideal and Premium, with class-specific accuracies of $88.7\%$ and $86.5\%$, respectively. This suggests that depth and table are especially informative for identifying higher-quality cuts, where geometric differences are more pronounced. In contrast, classification performance is considerably lower for Fair/Good ($21.1\%$) and Very Good ($25\%$) cuts.

Misclassifications occur mainly between neighboring cut categories. The amount of Fair/Good diamonds that are classified as Very Good is about twice the number of those correctly classified, and the same pattern is seen with Very Good diamonds being classified as Premium. This pattern is consistent with the ordinal nature of cut quality and indicates that the model captures the underlying structure of the data rather than producing arbitrary errors.

It should be highlighted that the model represents a clear improvement over random guessing. The baseline accuracy, computed from class proportions, is $30.4\%$, whereas the achieved CCR is substantially higher. Furthermore, the Q statistic rejects the null hypothesis of random classification. This confirms that the discriminant function extract meaningful information for cut classification, even with a limited set of predictors.

In conclusion, the results show that Quadratic Discriminant Analysis provides a robust and informative classification approach. Using only depth and table, the model achieves a reasonably accurate and interpretable prediction of diamond cut quality, particularly for distinguishing higher-grade cuts. This finding is fully consistent with the Gower-based MDS results, where cut categories, especially Ideal and Premium, exhibit a clear separation along the dimension associated with geometric proportions. In both analyses, these two cut types are characterized by distinct depth-table combinations, confirming that geometric balance plays a central role in differentiating cut quality and reinforcing the coherence between the discriminant and distance-based approaches.
