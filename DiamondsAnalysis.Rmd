...
Although the automatic HCPC cut suggests a 3-cluster partition, the factor map and the dendrogram reveal that a 4-cluster solution is more balanced and substantially more interpretable. The three-cluster solution produces one dominant group, whereas splitting the structure into four clusters yields groups of comparable size and a clearer separation in the PCA space.
...
-   **Cluster 4** (blue) appears on the far positive side of Dimension 1, representing the largest and most valuable diamonds, with shapes that are generally balanced or slightly deep.
...
The clustering analysis that includes both numerical and categorical variables is performed through a Factor Analysis of Mixed Data (FAMD), which provides a joint low-dimensional space where continuous and categorical variables contribute simultaneously to the definition of the principal axes.
...
#### Profiling based on Principal Components??
