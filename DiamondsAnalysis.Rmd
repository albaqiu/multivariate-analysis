...
The depth variable has 2 severe outliers, which we removed. The histogram of the variable shows that the depth variable. Moreover, the QQ-plot almost shows a straight line without major deviations, which suggests that this variable could be considered approximately normal. 

** We observe a long right tail in the histogram, and assessing with the boxplot and the outlier detection, we confirm that they are outliers. We remove them to make the distribution closer to normality.** 

Nonetheless, the shapiro and the kolmogorov test show a low p-value, leading to the rejection of the normality hypothesis.
...
```
ESTA TOT COMENTAT PQ ESPERARE A PREGUNTAR-LI A LA NIHAN SI VAL LA PENA O NO

Although it isn't an outlier, the distribution shows a very low value at the low values of x, which could be the reason why a log transformation isn't considered. We will analize whether removing this observation proves to be benefitial.
```{r}
# lowobs <- which(diamonds$x < 4)
# length(lowobs) # 3/496 observations is less than 1% - we can remove
# diamonds <- diamonds[-lowobs,] # Remove observation with low value
# summary(diamonds$x)
# 
# b <- boxcox(lm(diamonds$x~1)) # Box-Cox Transformation
# lambda <- b$x[which.max(b$y)]
# diamonds$newx <- (diamonds$x ^ lambda - 1) / lambda # lambda = -0.38 (transformation)
# 
# # Is newx normally distributed?
# hist(diamonds$newx, freq = F)
# m = mean(diamonds$newx)
# std = sd(diamonds$newx)
# curve(dnorm(x,m,std),col="red",lwd=2,add=T)
# 
# qqnorm(diamonds$newx, main = "newx Q-plot")
# qqline(diamonds$newx)
# 
# shapiro.test(diamonds$newx)
# ks.test(diamonds$newx, "pnorm", mean=mean(diamonds$newx), sd=sd(diamonds$newx))
...
We have decided to take price as a suplementary variable since the results are more interpretable. However, the initial PCA is included.
...
The logPrice variable has been treated as a supplementary variable. After performing the initial PCA using all numeric variables, we have found that including price as an active variable could dominate the principal components due to its high variance and strong correlations with the diamond’s physical dimensions. By treating price as supplementary, we can examine how price relates to the main components without affecting their structure. As a result, the extracted components are easier to interpret and focus on the physical properties of the diamonds, while still reflecting their relationship with price.
...
We apply CA only to the original categorical variables (cut, color, clarity), we will analyze this pairwisely: cut x color, cut x clarity, color x clarity.

In MCA, we include the numerical variables by binarizing them, but we do not run CA on these binary variables because MCA already captures their relationships and CA would not add useful information.

### Cut x color
There's 4 dimensions because dim = min(5−1, 7−1)

Analysis:
Dimension 1 (58.7%) represents a color quality axis:
- Premium cut diamonds are associated with poorer colors (G, I/J).
- Fair/Good diamonds associate with the best colors (D).
The categories that contribute more to this dimension are: 
- Premium cut and G color (negative)
- Fair/Good cut and D color (positive)

Dimension 2 (38.0%) captures differences between Ideal and Very Good:
- Ideal diamonds are more associated with D, F, and I/J colors.
- Very Good diamonds tend to be linked with H and E colors.
The categories that contribute more to this dimension are:
- Ideal and D, F, I/J (positive)
- Very Good and E, H (negative)
...
res.ca_cut_clarity <- CA(tab_cut_clarity)

# Eigenvalues
res.ca_cut_clarity$eig
barplot(res.ca_cut_clarity$eig[,2], main="Eigenvalues")

# Row profiles
res.ca_cut_clarity$row$coord
res.ca_cut_clarity$row$contrib
res.ca_cut_clarity$row$cos2

# Column profiles
res.ca_cut_clarity$col$coord
res.ca_cut_clarity$col$contrib
res.ca_cut_clarity$col$cos2

# Plot
plot.CA(res.ca_cut_clarity)
```
...
res.ca_color_clarity <- CA(tab_color_clarity)

# Eigenvalues
res.ca_color_clarity$eig
barplot(res.ca_color_clarity$eig[,2], main="Eigenvalues")

# Row profiles
res.ca_color_clarity$row$coord
res.ca_color_clarity$row$contrib
res.ca_color_clarity$row$cos2

# Column profiles
res.ca_color_clarity$col$coord
res.ca_color_clarity$col$contrib
res.ca_color_clarity$col$cos2

# Plot
plot.CA(res.ca_color_clarity)
```


## 2.4. Multiple correspondance analysis

In order to do Multiple Correspondance Analysis using also the numerical variables, these will be converted to binary variables with two categories as “over average” and “below average” for each of them, which will also help with its interpretability.
...
In this case, as there are 10 variables, J = 10, and the total number of values is K=5+7+8+(7x2)=34, number of dimensions is K-J=24. Therefore, the average inertia is 1/24 (0.04166667), and the dimensions selected would theoretically be until Dim.20.
...
```{r}
# K=5+7+8+(7x2)=34
# no of dimension: K-J=24
# average inertia per dimension: 1/(K-J)=1/24
summary(res.mca, ncp=4)
# Choose the dimensions with eigenvalues > 1/24 --> Until Dim.20
...
- For Dimension 1, the variables are `x`, `y`, `z` and `logCarat` as their value of R2 is around 0.9.
  It is also relevant to note that the supplementary variable `logPrice` is also very significant in this dimension, with a value of R2 of 0.78.
- For Dimension 2, the variables are `cut` and `table`, as their value of R2 is around 0.69. 
...
- Dimension 1: At the category level, the most positively correlated modalities are the “over average” categories of the most significant variables (`x`, `y`, `z` and `logCarat`), which display the highest absolute positive estimates. Conversely, the corresponding “under average” modalities appear on the negative side with the same magnitude but opposite sign. This symmetric structure indicates a clear separation of the observations according to whether their size-related measurements are above or below the average.

- Dimension 2: Among the categories, the “table_o” modality and the Premium cut level show the strongest positive correlation with this dimension. On the opposite side, the “table_b” category and the Ideal cut have large negative estimates, marking the strongest negative associations.

Although it may initially seem counter-intuitive that Premium and Ideal cuts, both generally considered high-quality categories, appear on opposite sides of Dimension 2, this separation does not reflect differences directly in cut quality. This indicates that Premium is more frequently associated with over-average table values, whereas Ideal is more often linked to under-average table values in the dataset. As a result, these two high-quality cuts are positioned far apart—not due to opposite quality levels, but due to contrasting geometric characteristics captured by Dimension 2.
